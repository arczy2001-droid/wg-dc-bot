...
DATA MODEL (SFMonitor in sf_auth.py reads these):
...
def get_notify_config(guild_id: int, world_name: str) -> Optional[dict]:
    """Public — sf_auth.py's SFMonitor calls this to route alerts. Uses a
    busy timeout because SFMonitor now calls it from up to
    MAX_CONCURRENT_PROBES parallel tasks."""
...
# channel/role pickers and a defense-mute toggle.
# ---------------------------------------------------------------------------
...
class AlertConfigView(discord.ui.View):
    ...
    The mute toggle is a PERSISTENT button — its custom_id embeds the world
    so it keeps working after a bot restart, re-registered via
    register_alert_panel_views(). Guild is taken from interaction.guild_id at
    click time (a button click always carries its guild).
    """
    ...
    async def _on_channel(self, interaction: discord.Interaction):
        ...
        role_id = existing[1] if existing else "0"
...
async def register_alert_panel_views(bot: discord.Client) -> None:
    ...
    config, so the button keeps working after a restart."""
    conn = sqlite3.connect(DB_PATH)
